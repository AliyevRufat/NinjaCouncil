...
bool SVGParser::GetVerticesFromSvgFile(const std::string& filePath, std::vector<std::vector<Point2f>>& vertices)
{
	...
	ScaleVertices(vertices, Point2f{ 2,2 });
	return true;
}
...
void SVGParser::ScaleVertices(std::vector<std::vector<Point2f>>& vertices, const Point2f& scale)
{
	for (std::vector<Point2f>& List : vertices)
	{
		for (Point2f& p2f : List)
		{
			p2f.x *= scale.x;
			p2f.y *= scale.y;
		}
